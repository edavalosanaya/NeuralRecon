...
from neuralrecon.datasets.sampler import DistributedSampler
from neuralrecon.ops.comm import is_main_process, synchronize
...
if __name__ == '__main__':
    ...
    args = get_args()
    cfg = nr.DF_CF.clone()
    nr.utils.update_config(cfg, args)
    ...
    transform = []
    transform += [nr.datasets.transforms.ResizeImage((640, 480)),
                  ...
                  nr.datasets.transforms.IntrinsicsPoseToProjection(n_views, 4),
                  ]

    nr.datasets.transforms = nr.datasets.transforms.Compose(transform)

    # dataset, dataloader
    MVSDataset = nr.datasets.find_dataset_def(cfg.DATASET)
    train_dataset = MVSDataset(cfg.TRAIN.PATH, "train", nr.datasets.transforms, cfg.TRAIN.N_VIEWS, len(cfg.MODEL.THRESHOLDS) - 1)
    test_dataset = MVSDataset(cfg.TEST.PATH, "test", nr.datasets.transforms, cfg.TEST.N_VIEWS, len(cfg.MODEL.THRESHOLDS) - 1)
        ...
        for epoch_idx in range(start_epoch, cfg.TRAIN.EPOCHS):
            ...
            for batch_idx, sample in enumerate(TrainImgLoader):
                ...
                if do_summary and is_main_process():
                    nr.utils.save_scalars(tb_writer, 'train', scalar_outputs, global_step)
                del scalar_outputs
        ...
        while True:
            ...
            for ckpt in saved_models:
                if ckpt not in ckpt_list:
                    ...
                    avg_test_scalars = nr.utils.DictAverageMeter()
                    save_mesh_scene = nr.utils.SaveScene(cfg)
                    ...
                    nr.utils.save_scalars(tb_writer, 'fulltest', avg_test_scalars.mean(), epoch_idx)
    ...
    def train_sample(sample):
        ...
        optimizer.step()
        return nr.utils.tensor2float(loss), nr.utils.tensor2float(loss_dict)


    @nr.utils.make_nograd_func
    def test_sample(sample, save_scene=False):
        ...
        return nr.utils.tensor2float(loss), nr.utils.tensor2float(loss_dict), outputs
